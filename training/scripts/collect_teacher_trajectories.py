"""Collect teacher (obs, action) transitions for BC pre-training."""
...
# Must set env before Isaac imports
os.environ.setdefault("STAGE3_TEACHER_MODE", "only")
os.environ.setdefault("STAGE3_REWARD_STAGE", "A")
...
def main() -> None:
    ...
    out = Path(args.output) if args.output else PROJECT_ROOT / "data" / "bc" / args.curriculum / "transitions.pt"
    ...
    torch.save(data, out)
    print(f"[BC Collect] Saved {data['obs'].shape[0]} transitions -> {out}", flush=True)
    env.close()
